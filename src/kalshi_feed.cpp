...
static const int MAX_BACKOFF_SECS = 60;
...
int main()
{
    ...
    while (true) {
        try {
            // --- Fresh signature each attempt (timestamp must be current) ---
            EVP_PKEY* pkey = load_private_key(pem_path);
            if (!pkey)
                throw std::runtime_error("Failed to load RSA key");
            ...
            tv.tv_sec = 30;
            tv.tv_usec = 0;
            setsockopt(beast::get_lowest_layer(ws).native_handle(),
            ...
            backoff = 1; // connected successfully, reset backoff
            ...
            backoff = std::min(backoff * 2, MAX_BACKOFF_SECS);
        }
    }
